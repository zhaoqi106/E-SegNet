...
def test(fold):
    path = "../output_synapse/unetr_pp/3d_fullres/Task002_Synapse/unetr_pp_trainer_synapse__unetr_pp_Plansv2.1/"  # Replace None by full path of "DATASET/unetr_pp_raw/unetr_pp_raw_data/Task002_Synapse/"
    infer_path = path  # Replace None by full path of "output_synapse"
    print(os.path.join(path, 'gt_niftis', 'subset_for_inference', '*nii.gz'))
    infer_list = sorted(glob.glob(os.path.join(path, "fold_0", 'validation_raw_postprocessed', '*nii.gz')))
    # print(infer_list)
    os.makedirs(os.path.join(path, 'gt_niftis', 'subset_for_inference'), exist_ok=True)
    for p in infer_list:
        name = os.path.basename(p)
        de_path = os.path.join(path, 'gt_niftis', 'subset_for_inference', name)
        if not os.path.exists(os.path.join(path, 'gt_niftis', 'subset_for_inference', name)):
            label_path = os.path.join(path,"gt_niftis",name)
            import shutil
            shutil.copyfile(label_path,de_path)
            print(f"{label_path} ----> {de_path} ")
